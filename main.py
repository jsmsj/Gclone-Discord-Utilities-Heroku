...
async def on_ready():
    ...
    print(bot.user.id)
    ver = await execute(["rclone","version"])
    print(ver)
    print("---------------------------------------------------------------------")
    status_swap.start()
...
async def clone(ctx,source=None,name=None, destination=secrets.DEFAULT_DESTINATION_ID):
    ...
    cmd = ["rclone", "copy", f"GC:{s1}", f"GC:{d1}", "--transfers", "50", "-vP", "--stats-one-line", "--stats=15s", "--ignore-existing", "--drive-server-side-across-configs", "--drive-chunk-size", "128M", "--drive-acknowledge-abuse", "--drive-keep-revision-forever"]
...
async def size(ctx,source=None):
    ...
    cmd = ["rclone", "size",f"GC:{s1}","--fast-list"]
...
async def move(ctx, source=None, destination=None):
    ...
    cmd=["rclone", "move", f"GC:{s1}", f"GC:{d1}", "--transfers", "50", "--tpslimit-burst", "50", "--checkers", "10", "-vP", "--stats-one-line", "--stats=15s", "--ignore-existing", "--drive-server-side-across-configs", "--drive-chunk-size", "128M", "--drive-acknowledge-abuse", "--drive-keep-revision-forever", "--fast-list"]
    out = await execute(cmd)
    final = out[-1989::]
    logs =f"```ml\n{final}\n```"
...
async def sync(ctx, source=None, destination=None):
    ...
    cmd = ["rclone", "sync", f"GC:{s1}", f"GC:{d1}", "--transfers", "50", "--tpslimit-burst", "50", "--checkers", "10", "-vP", "--stats-one-line", "--stats=15s", "--drive-server-side-across-configs", "--drive-chunk-size", "128M", "--drive-acknowledge-abuse", "--drive-keep-revision-forever", "--fast-list"]
    out = await execute(cmd)
    final = out[-1989::]
    logs =f"```ml\n{final}\n```"
...
async def deletefolder(ctx, source=None):
    ...
    cmd=["rclone", "delete", f"GC:{s1}", "-vP","--stats-one-line", "--stats=15s", "--fast-list"]
    out = await execute(cmd)
    final = out[-1989::]
    logs =f"```ml\n{final}\n```"
...
async def deletefile(ctx, source=None):
    ...
    cmd=["rclone", "deletefile", f"GC:{s1}", "-vP", "--drive-trashed-only","--stats-one-line", "--stats=15s", "--fast-list"]
    out = await execute(cmd)
    final = out[-1989::]
    logs =f"```ml\n{final}\n```"
...
@is_admin()
async def supero(ctx,cmd=None):
    if not cmd: return await ctx.send(f"Run {secrets.PREFIX}help bash For correct usage")
